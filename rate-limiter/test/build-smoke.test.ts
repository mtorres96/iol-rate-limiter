...
// store code into a shared top-level chunk in `dist/`, so that URL resolves to
// `dist/lua/` (NOT `dist/store/lua/`); the `onSuccess` hook in tsup.config.ts
// copies the `.lua` assets there. This test runs the real build and asserts
// those assets land at the path the runtime actually reads — catching a broken,
// removed, or mis-targeted asset-copy step before it silently ships a package
// that can't load its own scripts (the real-Redis path 500s only in the
// container otherwise — Pitfall 4).
...
describe("build ships the assets into dist", () => {
  beforeAll(() => {
    ...
    // `.lua` files into `dist/lua/`, and the second entry emits the Express
    // adapter into `dist/adapters/express/`.
    execFileSync("npm", ["run", "build"], { cwd: pkgRoot, stdio: "inherit" });
  }, 120_000);

  it.each(luaScripts)("dist/lua/%s exists and is non-empty", (script) => {
    const path = fileURLToPath(new URL(`../dist/lua/${script}`, import.meta.url));
